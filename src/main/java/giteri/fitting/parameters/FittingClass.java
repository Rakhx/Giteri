...
/** Classe de configuration pour lancer un giteri.fitting
 * ou autre recherche de stabilité.  
 * 
 * Run: Lancement d'un simulation en partant de zero niveau 
 * giteri.meme, état du réseau, etc
 * Step: Pendant un giteri.run, interval de temps entre deux mesures
 * Config // set de parametre : valeur des paramètres de configuration
 * de la simulation. Ne change pas pendant un giteri.run.
 *
 */
public class FittingClass implements BehaviorTransmissionListener, ActionApplyListener {
	...
	public CommunicationModel com = CommunicationModel.getInstance();
	public IExplorationMethod explorator;
	
	// Region CONFIGURATION INITIALE
	
	// Nombre de fois ou on lance un giteri.run pour une giteri.meme config.
	...
	// numero du giteri.run en cours
	public int numeroRun = -1;
	public String numeroRunAsString = "-1";
	
	// numéro de répétition dans le giteri.run
	public int numeroTurn = -1;
	public String numeroTurnAsString = "-1";
	
	// Compteur du nombre de relevé fait pour un giteri.meme giteri.run
	...
	// Cette liste est changée pendant le giteri.fitting a chaque event action
	public CircularFifoQueue<Meme> cqLastXActionDone;
	// Ces deux listes sont utilisées dans la boucle la plus interne du giteri.fitting, donc chaque nbActionByStep
...
//	private double threshHeCoeff;
	...
	// relevés des différentes densité pour une giteri.meme configuration
...
//	public double currentDensity;
	...
	Result resultNetwork = new Result();
	 ...
	 * @param stabilityRun
	 */
	public FittingClass(boolean stabilityRun){
		setDefaultValue();
	}
	...
	private void setDefaultValue(){
		...
		memesAvailables = MemeFactory.getInstance().getMemeAvailable(false);
		currentNetProperties.createStub();
	}
	
	// Region Fitting running
	
	/** Initialisation des variables nécessaire a un giteri.fitting.
	 * Ecriture dans les fichiers, ouverture des répertoires.
	 * 
	 */
	public void init(){
		
		targetNetProperties = NetworkFileLoader.getInstance().getNetworkProperties();
		...
		{
			// STEP: HEADER
			repOfTheSearch = WriteNRead.getInstance().createAndGetDirFromString(repertoires);
			String header = "Name";
			...
			WriteNRead.getInstance().writeSmallFile2(repOfTheSearch, "NetworkCSV", Arrays.asList(toWriteNormalCSV));
			...
			WriteNRead.getInstance().writeSmallFile2(repOfTheSearch, "NetworkDetailsCSV", Arrays.asList(toWriteDetailCSV));
		}
	}
	...
	/** Nouveau giteri.run, donc reset du giteri.network et pas de chg sur la config. courante du modèle.
	 * 
	 */
	public void newTurn(){
		resetSeuilValue();
	...
	/** Fin du giteri.run, une fois que le réseau a atteint un cycle//stabilité // chaos.
	 * Enregistre les valeurs, sauvegarde et snapshot. 
	 * 
	 */
	public void endTurn(){
		turnCount = 0;
		// STEP: Concernant la continuité du giteri.fitting sur giteri.meme config.
		nbCallContinuOnThisConfig = 0;
		resetSeuilValue();
		
		// STEP: On prend les properties courantes pour calculer une distance avec le réseau cible
		NetworkConstructor.getInstance().updateAllNetworkProperties();
		// passe par le tinyNetwork
		currentNetProperties = NetworkConstructor.getInstance().getNetworkProperties();
		
		// TODO [WayPoint]- Score distance entre deux giteri.network
		...
		// idée: distance entre les score des différents giteri.run exponentiel
		...
		resultNetwork.addScore(numeroRun, configAsString, currentNetworkScore, NetworkConstructor.getInstance().getNetworkProperties().getCopyMe());
	...
	/** Fin du tour, enregistre les variances sur les résultats des différents giteri.run sur giteri.meme config,
	 * 
	 * 
	 */
	public void endRun(){
...
//		explorator.rememberNetwork(currentNetworkId);
	...
	/** Version utilisée pour savoir s'il faut continuer a faire du giteri.fitting ou si on a
	...
	public boolean continuFittingCleanVersion(){
		...
		// REGION Vérification état du giteri.network
		...
		if(canStillApplyAction){
			...
			if(useDensity)
			{ 
				// meanDensityValueOnOneRun est mise a jour dans la boucle du giteri.fitting avant l'appel de cette fonction
		...
		if(canStillApplyAction)
		{
			...
			if(useMemeAppliance)
			{
				...
				// Définition du hard et soft pour le giteri.meme appliance
	...
	private boolean readingActionCanContinue(ObjectRef<String> message){
		...
		if(nbActionLocal == 0){
			...
			if(!EntiteHandler.getInstance().forceAction()){
				resultat = false;
	 ...
	 * 
	 * @param densities
	 * @param hardEndDensityVariation
	 * @param softEndDensityVariation
	 * @return
	 */
	 ...
	 * 
	 * @param densities
	 * @param hardendCoeffDir
	 * @param softendCoeffDir
	 * @param message
	 * @return
	 */
	 ...
	 * Fonction écrite pour pouvoir utiliser les proportions d'applications OU l'écart type d'application d'un giteri.meme
	 * sur les X derniers appels. Aucune fonction de score pour l'utilisation de nombre de giteri.meme appliqué
	 * Entre chaque appel de cette fonction 500 actions sont lancé dans le giteri.fitting ( pas necessairement
	 * sucessful )
	 * @param displayEcartTypeOnGraph si true display l'écart type, sinon display les proportions d'application par giteri.meme
	 * @param message
	 * @return
	 */
	private double readingMemeAppliance(boolean displayEcartTypeOnGraph, ObjectRef<String> message){

		// Le relevé des memes appliances est fait dans la boucle de giteri.fitting donc pas de risque qu'elle soit modifier
		...
		// On regarde sur les CircularSize dernieres actions jouées la proportion pour un giteri.meme, copie en local
		// et mis a zero si le giteri.meme n'est jamais joué. // TODO [Refact.2] DES STREAMS
		...
		// Ajout dans une circular queue qui sera rempli pendant les appel de cette fonction sur le giteri.meme giteri.run
		...
		// contient les prop. d'application ou l'écart type de l'application d'un giteri.meme
		Hashtable<Meme, Double> kvEcartTypeOrPropMemeAppliance = new Hashtable<Meme, Double>();

		// Dans le cas ou on veut l'écart type de l'application d'un giteri.meme par rapport à sa propre moyenne
		...
		if(displayEcartTypeOnGraph){
			...
			for (Hashtable<Meme,Integer> setOfMemeNumberForOneStep : cfqMemeApplianceNumberOnFitting){ 
				// on regarde chaque giteri.meme a chaque step le nombre de fois ou il a été appelé
		...
		// Lecture des N dernier prop de giteri.meme et placement dans une liste pour analyser les écarts
		else {
		...
		// de giteri.meme. la fonction getIndiceEvolution renvoie 4 si pas assez de valeur pour regarder
		// l'évolution
		double maxValue = 4.;
		double localScore ;
		...
		for (Meme meme : stabilityByMeme.keySet()) {
			...
			// la valeur maximum d'un score pour un giteri.meme est (100 / nbMeme) pour que le score final en cas d'instabilité
			// maximum pour tous les memes a été sommé à 100. 
			localScore = localScore * (100 / (memesAvailables.size() * maxValue)) ;
			resume  += ("Meme "+ EntiteHandler.getInstance().translateMemeCombinaisonReadable(meme.toFourCharString())
			+ " score normalisé entre 0 et "+ 100 / memesAvailables.size() +" = " + localScore);
			score += localScore;
		}
		
		// Affichage dans la fenetre de l'évolution des giteri.meme appliances
		Hashtable<Integer, Double>  kvMemeValue = new Hashtable<Integer, Double>() ;
		for (Meme meme : kvEcartTypeOrPropMemeAppliance.keySet()) 
			kvMemeValue.put(MemeFactory.getInstance().getColorIndex(meme), kvEcartTypeOrPropMemeAppliance.get(meme));
	...
	public static double getNetworksDistanceDumb(int activationCode, NetworkProperties targetNetworkProperty, NetworkProperties currentNetworkProperty) {
		...
		for (int i = 0; i < Configurator.NetworkAttribType.values().length; i++) {
			...
			{
				// Calcul de la distance entre deux giteri.network sur ce critère
	...
	/** Reset des seuils qui définissent la fin d'un round de giteri.fitting
	 * 
	 */
...
//		threshHeCoeff = 0.001;
	...
	/** Augmente le treshold des valeurs pour les soft et hard fin de giteri.run
	 * 
	 */
...
//		threshHeCoeff *= (1 + risingPourcentage) ;
	...
	// Region Ensemble de méthode pour compter le nombre d'action réalisée dans le giteri.network.
	
	/** Implemente ces handler d'entités pour pouvoir compter le nombre
	 * d'actions réalisées afin d'etre en mesure de connaitre la progression du giteri.network.
	 * 
	 */
