...
public abstract class StatAndPlotGeneric implements StatAndPlotInterface {
	...
	// Empeche l'algo de passer au step suivant de giteri.fitting sans le passage manuel
	...
	public void callFactorisedFunction(boolean stabilityRun){
		...
		// ajout de la giteri.fitting classe au listener
...
//		EntiteHandler.getInstance().addMemeListener(configuration);
		...
		// retrait de la giteri.fitting classe des listener
		EntiteHandler.getInstance().removeEntityListener(configuration);
//		EntiteHandler.getInstance().removeMemeListener(configuration);
	}

	/** Initialise et instancie pour préparer le giteri.fitting. Behavior présent sur la map,
	...
	/** Classe factorisée pour les traitements de giteri.fitting ou searching.
	 * 
	 * @param config
	 */
