...
//		memeAndProba.entrySet().stream().map(( v -> v.getKey().toFourCharString().concat(v.getValue().valueString()))).collect(Collectors.toList()));
	...
	private void generateProxyParam(List<Boolean> activator, List<Double> proba) {
		List<Integer> toConvert = new ArrayList<>(Arrays.asList(1,1,0,0,1,1,1,0,1,1,0,0,1));
		proba.addAll(Arrays.asList(
				0.2971083615619568,0.5632075767419494,0.7402183233880577,0.9850575238266355,0.09368830076032042,0.0048203963902108335,0.24523459337289025,0.7025623186859916,0.16677709490982282,1.0,0.5910680326337846,0.1506179573635582,0.8214570995344461
